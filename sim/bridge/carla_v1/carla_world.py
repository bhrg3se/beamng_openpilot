...
from openpilot.tools.sim.bridge.beamng_v3.beamng_process import beamng_process
...
class BeamNGWorld(World):
  def __init__(self, status_q, config, test_duration, test_run, dual_camera=False):
    ...
    self.bng = BeamNGpy(host='172.30.112.1', port=64256,home="/mnt/c/Program Files/BeamNG.tech.v0.32.2.0",quit_on_close=True,)
    # Launch BeamNG.tech
    ...
    self.beamng_process = multiprocessing.Process(name="beamng process", target=functools.partial(beamng_process,self.vehicle,self.dash_cam,self.camera_array,self.control_q,self.sensors_q,self.image_lock))
    self.beamng_process.start()
    # threading.Thread(target=self.read_cameras_t).start()

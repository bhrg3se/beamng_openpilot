...
from openpilot.tools.sim.bridge.beamng_v2.beamng_process import (beamng_process, beamng_simulation_state,
                                                                    beamng_vehicle_state)
...
class BeamNGWorld(World):
  def __init__(self, status_q, config, test_duration, test_run, dual_camera=False):
    ...
    self.beamng_process = multiprocessing.Process(name="beamng process", target=
                              functools.partial(beamng_process, dual_camera, config,
    ...
    self.vehicle_last_pos = self.vehicle_state_recv.recv().position # wait for a state message to ensure beamng is launched
  ...
  def read_state(self):
    while self.simulation_state_recv.poll(0):
      md_state: beamng_simulation_state = self.simulation_state_recv.recv()
      if md_state.done:
  ...
  def read_sensors(self, state: SimulatorState):
    while self.vehicle_state_recv.poll(0):
      md_vehicle: beamng_vehicle_state = self.vehicle_state_recv.recv()
